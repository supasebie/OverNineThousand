  ...
  return (
    <div className="relative min-h-screen font-mono">
      <div className="max-w-5xl mx-auto p-6">
          ...
          {apps.map((app) => (
            <Link
              ...
              className="group block p-6 rounded-xl hover:bg-slate-50 border transition-all 
                       hover:border-purple-500/50 hover:shadow-lg bg-white"
            >
              <div className="flex justify-between items-start mb-4">
                <h3 className="font-medium text-xl group-hover:text-purple-500 transition-colors">{app.name}</h3>
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed">{app.description}</p>
              <div className="mt-4 text-sm text-purple-500 font-medium opacity-0 group-hover:opacity-100 transition-opacity">
                Try it now →
              </div>
            </Link>
          ))}
        ...
        {/* Back to Home */}
        <div className="mt-8">
          <Link href="/" className="flex items-center gap-2 text-sm hover:text-purple-500 transition-colors">
            <span>←</span>
